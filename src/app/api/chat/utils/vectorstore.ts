...
export async function loadVectorStore(pdfDirPath: string): Promise<FaissStore> {
  if (existsSync(VECTOR_STORE_PATH)) {
    const db = await FaissStore.load(VECTOR_STORE_PATH, embeddings);
  ...
  console.log("Created and saved new FAISS vector store");
  return db;
}
