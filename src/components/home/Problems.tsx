          ...
          {problems.map(
            ({ icon, color, bg, title, description, animation }, i) => (
              <Card
                key={i}
                className={cn(
                  `bg-gradient-to-br ${bg} p-8 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300`,
                  animation
                )}
              >
                <div
                  className={cn(
                    `w-16 h-16 bg-gradient-to-r ${color} rounded-full flex items-center justify-center mb-6 mx-auto`
                  )}
                >
                  {icon}
                </div>
